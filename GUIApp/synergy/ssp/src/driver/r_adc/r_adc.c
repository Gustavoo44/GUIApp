...
static ssp_err_t r_adc_sensor_cfg_temperature(ADC_BASE_PTR              const p_regs,
                                              adc_channel_cfg_t const * const p_cfg);

static void r_adc_close_sub(ADC_BASE_PTR          const p_regs,
                            adc_instance_ctrl_t * const p_ctrl);

static void r_adc_open_sub(ADC_BASE_PTR      const p_regs,
                           adc_cfg_t const * const p_cfg);

static ssp_err_t r_adc_sensor_cfg(ADC_BASE_PTR              const p_regs,
                                  adc_channel_cfg_t const * const p_cfg);
                                              ...
                                              adc_channel_cfg_t const * const p_cfg)
{
    ...
    HW_ADC_TemperatureSensorCfg(p_regs, temperature_add);
                            ...
                            adc_instance_ctrl_t * const p_ctrl)
{
    ...
    if (0 != (p_ctrl->scan_mask & ADC_MASK_TEMPERATURE))
    {
        if (NULL != gp_tsn_regs)
        {
            ...
            HW_ADC_TempSensorDisable(gp_tsn_regs);
        }
                           ...
                           adc_cfg_t const * const p_cfg)
{
    ...
    HW_ADC_ScanModeSet(p_regs, p_cfg->mode);

    /** Disable double trigger since this is not currently supported */
    HW_ADC_DoubleTriggerSet(p_regs, HW_ADC_DOUBLE_TRIGGER_DESELECTED);

    /** Configure trigger if its a hardware trigger*/
    if (ADC_TRIGGER_SYNC_ELC == p_cfg->trigger)
    {
        /** Set the ELC value for normal/groupA trigger*/
        HW_ADC_ElcTriggerGroupASet(p_regs, ADC_ELC_TRIGGER);
    }
    ...
    else if (ADC_TRIGGER_ASYNC_EXT_TRG0 == p_cfg->trigger)
    {
        ...
        HW_ADC_AsyncTriggerEnable(p_regs);
        /** Set TRSA to 0x00 if using Asynchronous external trigger*/
        HW_ADC_ElcTriggerGroupASet(p_regs, ADC_ELC_TRIGGER_EXTERNAL);
    }
    else
    {
                                  ...
                                  adc_channel_cfg_t const * const p_cfg)
{
    ...
    if ((p_cfg->scan_mask & ADC_MASK_TEMPERATURE) || (p_cfg->scan_mask_group_b & ADC_MASK_TEMPERATURE))
    {
        ...
        err = r_adc_sensor_cfg_temperature(p_regs, p_cfg);
        if (SSP_SUCCESS != err)
        {
            return err;
        }
                                ...
                                uint32_t          const * const p_valid_channels)
{
    ...
    {
        err = r_adc_sensor_cfg(p_regs, p_cfg);
        if (SSP_SUCCESS != err)
        {
            return err;
        }
